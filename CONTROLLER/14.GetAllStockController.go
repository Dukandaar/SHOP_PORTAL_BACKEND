...
func GetAllStock(ctx iris.Context) {
	...
	reqBody, bodyError := utils.ReadAllStockBody(ctx)
	utils.Logger.Info(logPrefix, headers, qparams, reqBody)

	headerError, errCodeStr := validator.ValidateHeader(utils.GetAllStockHeaders, headers, ctx, logPrefix)
	if errCodeStr != utils.SUCCESS { // header error
		...
		QparamsError, errCodeStr := validator.ValidateQParams(utils.GetAllStockQParams, qparams, logPrefix)
		if errCodeStr != utils.SUCCESS { // qparams error
			...
			reqBodyError, errCodeStr := validator.ValidateAllStockBody(&reqBody, bodyError)
