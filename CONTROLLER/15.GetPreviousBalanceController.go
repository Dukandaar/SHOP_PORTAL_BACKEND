...
func GetPreviousBalance(ctx iris.Context) {
	...
	headerError, errCodeStr := validator.ValidateHeader(utils.GetPreviousBalanceHeaders, headers, ctx, logPrefix)
	if errCodeStr != utils.SUCCESS { // header error
		...
		QparamsError, errCodeStr := validator.ValidateQParams(utils.GetPreviousBalanceQParams, qparams, logPrefix)
