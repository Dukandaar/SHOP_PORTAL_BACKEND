...
func GetAllCustomerBill(ctx iris.Context) {
	...
	if rspCode == utils.StatusOK {
		response, rspCode = validator.ValidateHeader(utils.GetAllCustomerBillHeaders, headers, ctx, logPrefix)
		if rspCode == utils.StatusOK {
			response, rspCode = validator.ValidateQParams(utils.GetAllCustomerBillQParams, qparams, logPrefix)
			if rspCode == utils.StatusOK {
