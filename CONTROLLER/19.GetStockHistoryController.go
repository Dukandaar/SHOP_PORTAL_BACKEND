...
func GetStockHistory(ctx iris.Context) {
	...
	headerError, errCodeStr := validator.ValidateHeader(utils.GetStockHistoryHeaders, headers, ctx, logPrefix)
	if errCodeStr != utils.SUCCESS { // header error
		...
		QparamsError, errCodeStr := validator.ValidateQParams(utils.GetStockHistoryQParams, qparams, logPrefix)
