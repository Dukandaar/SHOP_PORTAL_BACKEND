...
func PutShopOwner(reqBody structs.ShopOwner, OwnerRegId string, logPrefix string) (interface{}, int) {
	...
	if err != nil && err != sql.ErrNoRows {
		response, rspCode = helper.Create500ErrorResponse("Error in getting row", "Error in getting row:"+err.Error(), logPrefix)
	...
	if isActive != utils.NULL_STRING {
		if reg_id == OwnerRegId {
			...
			response, rspCode = helper.CreateErrorResponse("400009", "Another owner with same name, shopname and phone_no. exists : reg_id : "+OwnerRegId, logPrefix)
			return response, rspCode
		}
	}
	...
	if err != nil {
		response, rspCode = helper.Create500ErrorResponse("Error in updating row", "Error in updating row:"+err.Error(), logPrefix)
	} else {
		response, rspCode = helper.CreateSuccessResponse("Updated Successfully.", "Updated owner with reg_id : "+OwnerRegId)
	}
