...
func PostShopOwner(reqBody structs.ShopOwner, logPrefix string) (interface{}, int) {
	...
	if err == sql.ErrNoRows { // Shop owner NOT found (proceed with insertion)
		...
		if err != nil {
			return helper.Create500ErrorResponse("[DB ERROR 0007] Error in inserting Shop Owner Data", "Error inserting Shop Owner Data: "+err.Error(), logPrefix)
		}
		...
		if err != nil {
			return helper.Create500ErrorResponse("[DB ERROR 0008] Error in inserting Shop Owner Balance Data", "Error inserting Shop Owner Balance Data: "+err.Error(), logPrefix)
		}
		...
		if err != nil {
			return helper.Create500ErrorResponse("[DB ERROR 0009] Error in inserting Shop Owner Bill Count Data", "Error inserting Shop Owner Bill Count Data: "+err.Error(), logPrefix)
		}
		...
		return helper.Create500ErrorResponse("[DB ERROR 0010] Error checking Shop Owner Data", "Error checking Shop Owner Data: "+err.Error(), logPrefix)
	} else {
		...
		if err != nil {
			return helper.Create500ErrorResponse("[DB ERROR 0011] Error in updating Shop Owner data", "rror in updating Shop Owner data: "+err.Error(), logPrefix)
		}
	...
	if rspCode == utils.StatusOK {
		err = tx.Commit()
		if err != nil {
			return helper.Create500ErrorResponse("[DB ERROR 0012] Error in committing transaction", "Error in committing transaction: "+err.Error(), logPrefix)
		}
